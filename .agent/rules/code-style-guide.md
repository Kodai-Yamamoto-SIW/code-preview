...
- **テストツール**: Playwright Component Testing を採用しています。
- **テストの実行**: コードの変更を行った際は、必ず `npm run test` でテストを実行し、既存の機能が破壊されていないか確認してください。テストの結果は、コンソールに出力されるので、そこで確認してください。テストはすべてのテストが成功した場合、自動で終了します。失敗がある場合は、終了すると `Press Ctrl+C to quit` と表示されるので、それを確認したら、`Ctrl+C` を押してテストランナーを終了してください。
- **静的解析 (Lint)**: ソースコードを変更した際は、必ず `npm run lint` を実行してください。警告やエラーが発生した場合は、それらが完全に解決されるまで修正を行ってください。
- **テストの追加**: 新しい機能やコンポーネントを追加した場合は、対応する E2E/コンポーネントテスト (`*.spec.tsx`) を作成・更新してください。また、既存の機能を修正した場合も、その修正内容を検証するテストを追加または修正してください。
- **1テスト1マウントの原則**: Playwright Component Testing では、**1つのテスト内で `mount()` を呼び出せるのは1回のみ**です。同じコンテナに複数回マウントしようとすると `Attempting to mount a component into an container that already has a React root` エラーが発生します。複数のシナリオをテストする場合は、テストケースを分割してください。
